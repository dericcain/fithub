...
export const Schedule = view(() => {
  useEffect(() => {
    ...
    Events.fetchTraineeEvents(User.displayName);
  }, []);
  ...
  const renderItem = ({
    ...
    const avatarProps = {};
    if (trainerAvatar) {
      avatarProps.source = { uri: trainerAvatar };
    } else {
      avatarProps.title = initials(participant);
    ...
    return (
      <Event isFithubEvent={!!participant}>
        <EventLeft>
          ...
          <EventName>
            <EventNameText>FITHUB: {name}</EventNameText>
          </EventName>
          <EventDetails>
            <EventDetailsText>
        ...
        <EventRight>
          {participant && <Avatar rounded size="small" {...avatarProps} />}
        </EventRight>
      </Event>
    );
  };
